...
import { ACCEPT_INVITATION_REQUEST, ACCEPT_INVITATION_SUCCESS, CREATE_PROJECT_REQUEST, CREATE_PROJECT_SUCCESS, DELETE_PROJECT_REQUEST, DELETE_PROJECT_SUCCESS, FETCH_PROJECT_BY_ID_REQUEST, FETCH_PROJECT_BY_ID_SUCCESS, FETCH_PROJECTS_REQUEST, FETCH_PROJECTS_SUCCESS, INVITE_TO_PROJECTS_REQUEST, INVITE_TO_PROJECTS_SUCCESS, SEARCH_PROJECTS_REQUEST, SEARCH_PROJECTS_SUCCESS } from "./ActionTypes"
...
export const inviteToProject = ({email,projectId}) => async (dispatch) => {
    ...
    try {
        ...
        dispatch({ type: INVITE_TO_PROJECTS_SUCCESS, payload: data}) //we don't need to handle state using (..., id) data 46min or we can hadnle it with payload
    } catch (error) {
        console.log("error = ", error);
